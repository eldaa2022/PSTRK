...
<head>
  ...
  <style>
    ...
    flex-direction: column; /* Membuat elemen di dalamnya tersusun secara vertikal */
}

#popup video {
    max-width: 90%;
    max-height: 70vh;
    margin-bottom: 10px; /* Memberi jarak antara video dan informasi */
}

#popup-info {
    background-color: white;
    width: auto; /* Mengatur lebar sesuai konten */
    max-width: 70%;
    text-align: center;
    padding: 10px;
    box-sizing: border-box;
    border-radius: 5px; /* Menambahkan border-radius untuk tampilan lebih lembut */
    display: inline-block; /* Membuat elemen mengikuti lebar konten */
}
        ...
        <div class="row">
          ...
          <div class="col-8">
            <div class="mb-4 pembatas-prestasi2">
              <h5 class="text-pembatas-prestasi">Galeri Video</h5>
              <img src="assets/img/line 24.png" alt="">
            </div>

            <div class="galeri-container-video mb-4">
                @foreach($videos as $video)
                <div class="galeri-item">
                    <div class="galeri-video" onclick="openPopup('{{ url('storage/video/'.$video->lampiran) }}')">
                        <video class="video-thumbnail" width="200px" height="200px">
                            <source src="{{ url('/storage/video/'.$video->lampiran) }}" type="video/mp4">
                          </video>
                    </div>
                    <div class="galeri-info-video">
                        <h6>{{ $video->judul }}</h6>
                        <p style="font-size: 12px">{{ $video->tgl_publish }}</p>
                    </div>
                </div>
                @endforeach
            </div>
...
<div class="modal fade" id="pesanModal" tabindex="-1" aria-labelledby="pesanModalLabel" aria-hidden="true">
          ...
          success: function (response) {
            ...
            Swal.fire({
              ...
              setTimeout(() => {
                ...
                window.location.href = "/videoPengunjung";
  ...
  <script>
    function openPopup(src) {
      ...
      var popupVideoSource = document.getElementById("popup-video-source");
