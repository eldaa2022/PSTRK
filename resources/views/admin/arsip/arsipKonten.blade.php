...
@section('judul_header', 'Arsip Konten')
        ...
        <div class="container">
            <div class="table-responsive">
                <div class="mt-4" style="height: 60px">
                    <form action="/arsipkonten" class="form-inline w-50 float-start" method="GET">
                        @csrf
                ...
                <table class="table">
                    ...
                    <tbody id="table-konten">
                        ...
                        <tr id="index_{{ $konten->id }}">
                            <td style="text-align: center">{{ ($kontens->currentPage() - 1) * $kontens->perPage() + $loop->iteration }}</td>
                            <td>{{$konten->judul}}</td>
                            <td>{{$konten->jenis}}</td> <!-- Menampilkan nilai jenis dari join -->
                            <td>{{$konten->tags}}</td>
                ...
                <nav aria-label="Page navigation">
                    <ul class="pagination">
                        <!-- Previous Page Link -->
                        <li class="page-item {{ $kontens->onFirstPage() ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $kontens->previousPageUrl() }}">Previous</a>
                        </li>

                        <!-- Pagination Elements -->
                        @foreach ($kontens->getUrlRange(1, $kontens->lastPage()) as $page => $url)
                            <li class="page-item {{ $page == $kontens->currentPage() ? 'active' : '' }}">
                                <a class="page-link" href="{{ $url }}">{{ $page }}</a>
                            </li>
                        @endforeach

                        <!-- Next Page Link -->
                        <li class="page-item {{ !$kontens->hasMorePages() ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $kontens->nextPageUrl() }}">Next</a>
                        </li>
                    </ul>
                </nav>
            </div>
