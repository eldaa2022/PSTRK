        ...
        <div class="container">
            <div class="table-responsive">
                <div class="mt-4" style="height: 60px">
                    <form action="/profil" class="form-inline w-50 float-start" method="GET">
                        @csrf
                ...
                <table class="table">
                    ...
                    <tbody id="table-konten">
                        @foreach ($profils as $index =>$konten)
                        <tr id="index_{{ $konten->id }}">
                            <td style="text-align: center;border-bottom: none">{{ ($profils->currentPage() - 1) * $profils->perPage() + $loop->iteration }}</td>
                ...
                <nav aria-label="Page navigation">
                    <ul class="pagination">
                        <!-- Previous Page Link -->
                        <li class="page-item {{ $profils->onFirstPage() ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $profils->previousPageUrl() }}">Previous</a>
                        </li>

                        <!-- Pagination Elements -->
                        @foreach ($profils->getUrlRange(1, $profils->lastPage()) as $page => $url)
                            <li class="page-item {{ $page == $profils->currentPage() ? 'active' : '' }}">
                                <a class="page-link" href="{{ $url }}">{{ $page }}</a>
                            </li>
                        @endforeach

                        <!-- Next Page Link -->
                        <li class="page-item {{ !$profils->hasMorePages() ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $profils->nextPageUrl() }}">Next</a>
                        </li>
                    </ul>
                </nav>
            </div>
