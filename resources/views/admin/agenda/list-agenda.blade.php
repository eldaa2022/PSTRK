            ...
            <div class="container">
                <div class="table-responsive">
                    <div class="mt-4" style="height: 60px">
                        <form action="/agenda" class="form-inline w-50 float-start" method="GET">
                            @csrf
                    ...
                    <table class="table">
                        ...
                        <tr id="index_{{ $agenda->id }}">
                                ...
                                <td class="text-center" style="padding-right:10px"> <a href="javascript:void(0)" id="btn-edit-post" data-id="{{ $agenda->id }}" class="button-edit btn btn-sm">edit</a> </td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                    <nav aria-label="Page navigation">
                        <ul class="pagination">
                            ...
                            <li class="page-item {{ $agendas->onFirstPage() ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $agendas->previousPageUrl() }}" style="te">Previous</a>
                            </li>

                            <!-- Pagination Elements -->
                            @foreach ($agendas->getUrlRange(1, $agendas->lastPage()) as $page => $url)
                                <li class="page-item {{ $page == $agendas->currentPage() ? 'active' : '' }}">
                            ...
                            <li class="page-item {{ !$agendas->hasMorePages() ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $agendas->nextPageUrl() }}">Next</a>
                            </li>
                        </ul>
                    </nav>
                </div>
